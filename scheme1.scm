...
(define (extend-environment vars vals base-env)
  (define (add-frame rev-vars rev-vals)
    (cons (make-frame (reverse rev-vars) (reverse rev-vals)) base-env))
  (let loop ((vrs vars)
             (vls vals)
             (rev-vrs '())
             (rev-vls '()))
    (if (variable? vrs)
        (add-frame (cons vrs rev-vrs) (cons vls rev-vls))
        (if (null? vrs)
            (if (null? vls)
                (add-frame rev-vrs rev-vls)
                (error (str "Too many arguments supplied: " vars vals)))
            (if (null? vls)
                (error (str "Too few arguments supplied: " vars vals))
                (loop (cdr vrs)
                      (cdr vls)
                      (cons (car vrs) rev-vrs)
                      (cons (car vls) rev-vls)))))))
